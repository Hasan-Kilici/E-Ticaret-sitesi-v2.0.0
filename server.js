...
Urunler.find().sort({createdAt : -1}).limit(4).then((urunSonuc)=>{
Urunler.find().count().then((UrunSayiSonuc)=>{
Urunler.find({kategori : "evesyalari"}).limit(4).then((evEsyalariSonuc)=>{
Urunler.find({kategori : "evesyalari"}).count().then((evEsyalariSayiSonuc)=>{
Urunler.find({kategori : "meyveSebze"}).limit(4).then((meyveSebzeSonuc)=>{
Urunler.find({kategori : "meyveSebze"}).count().then((meyveSebzeSayiSonuc)=>{
Urunler.find({kategori : "kozmetik"}).limit(4).then((kozmetikSonuc)=>{
Urunler.find({kategori : "kozmetik"}).count().then((kozmetikSayiSonuc)=>{
Urunler.find({kategori : "elektronik"}).limit(4).then((elektronikSonuc)=>{
Urunler.find({kategori : "elektronik"}).count().then((elektronikSayiSonuc)=>{
if(userId != null){
Kullanicilar.findById(userId).then((kullaniciSonuc)=>{
res.render(__dirname+'/src/signed/index.ejs',{title : "Anasayfa", kullanici : kullaniciSonuc, urun : urunSonuc, evesyalari : evEsyalariSonuc, meyvesebze : meyveSebzeSonuc, kozmetik : kozmetikSonuc, elektronik : elektronikSonuc, urunsayi : UrunSayiSonuc, evesyalarisayi : evEsyalariSayiSonuc, meyvesebzesayi : meyveSebzeSayiSonuc, kozmetiksayi : kozmetikSayiSonuc, elektroniksayi : elektronikSayiSonuc})
}).catch((err)=>{
res.render(__dirname+'/src/pages/index.ejs',{title : "Anasayfa", urun : urunSonuc, evesyalari : evEsyalariSonuc, meyvesebze : meyveSebzeSonuc, kozmetik : kozmetikSonuc, elektronik : elektronikSonuc, urunsayi : UrunSayiSonuc, evesyalarisayi : evEsyalariSayiSonuc, meyvesebzesayi : meyveSebzeSayiSonuc, kozmetiksayi : kozmetikSayiSonuc, elektroniksayi : elektronikSayiSonuc})
})
} else {
res.render(__dirname+'/src/pages/index.ejs',{title : "Anasayfa", urun : urunSonuc, evesyalari : evEsyalariSonuc, meyvesebze : meyveSebzeSonuc, kozmetik : kozmetikSonuc, elektronik : elektronikSonuc, urunsayi : UrunSayiSonuc, evesyalarisayi : evEsyalariSayiSonuc, meyvesebzesayi : meyveSebzeSayiSonuc, kozmetiksayi : kozmetikSayiSonuc, elektroniksayi : elektronikSayiSonuc});  
}
...
Siparis.find({kullanici : kullaniciSonuc.kullanici_adi}).then((SiparisSonuc)=>{
Siparis.find({kullanici : kullaniciSonuc.kullanici_adi}).count().then((SiparisSayi)=>{
if(userId != null){
res.render(__dirname+'/src/signed/sepet.ejs',{title : kullaniciSonuc.kullanici_adi+" Sepet",sepet : sepetSonuc, kullanici : kullaniciSonuc, urunsayi : sepetSayiSonuc, siparis : SiparisSonuc, siparissayi : SiparisSayi })
} else {
res.redirect('/')
}
})
...
Urunler.find({kategori : "evesyalari"}).count().then((evEsyalariSonucSayi)=>{
if(userId != null){
Kullanicilar.findById(userId).then((kullaniciSonuc)=>{
res.render(__dirname+'/src/signed/ev-esyalari.ejs',{title : "Anasayfa", kullanici : kullaniciSonuc, urun : urunSonuc, evesyalari : evEsyalariSonuc, evesyalarisayi : evEsyalariSonucSayi})
}).catch((err)=>{
res.redirect("/")
})
} else {
res.render(__dirname+'/src/pages/ev-esyalari.ejs',{title : "Anasayfa", urun : urunSonuc, evesyalari : evEsyalariSonuc, evesyalarisayi : evEsyalariSonucSayi});  
}
})
})
})
...
if(userId != null){
Kullanicilar.findById(userId).then((kullaniciSonuc)=>{
res.render(__dirname+'/src/signed/meyve-sebze.ejs',{title : "Anasayfa", kullanici : kullaniciSonuc, urun : urunSonuc, meyvesebze : meyveSebzeSonuc, meyvesebzesayi : meyveSebzeSayiSonuc, })
}).catch((err)=>{
res.redirect("/")
})
} else {
...
if(userId != null){
Kullanicilar.findById(userId).then((kullaniciSonuc)=>{
res.render(__dirname+'/src/signed/kozmetik.ejs',{title : "Anasayfa", kullanici : kullaniciSonuc, urun : urunSonuc, kozmetik : kozmetikSonuc, kozmetiksayi : KozmetikSayiSonuc})
}).catch((err)=>{
res.redirect("/")
})
} else {
res.render(__dirname+'/src/pages/kozmetik.ejs',{title : "Anasayfa", urun : urunSonuc, kozmetik : kozmetikSonuc, kozmetiksayi : KozmetikSayiSonuc});  
}
})
})
})
...
if(userId != null){
Kullanicilar.findById(userId).then((kullaniciSonuc)=>{
res.render(__dirname+'/src/signed/elektronik.ejs',{title : "Anasayfa", kullanici : kullaniciSonuc, urun : urunSonuc, elektronik : elektronikSonuc, elektroniksayi : elektronikSayiSonuc})
}).catch((err)=>{
res.redirect("/")
})
} else {
res.render(__dirname+'/src/pages/elektronik.ejs',{title : "Anasayfa", urun : urunSonuc, elektronik : elektronikSonuc, elektroniksayi : elektronikSayiSonuc});  
}
})
})
})
...
Yorumlar.find({urunId : id}).sort({createdAt : -1}).then((YorumSonuc)=>{
Yorumlar.find({urunId : id}).count().then((YorumSayi)=>{
if(userId != null){
Kullanicilar.findById(userId).then((kullaniciSonuc)=>{
res.render(__dirname+'/src/signed/urun.ejs',{title : urunSonuc.baslik, kullanici : kullaniciSonuc, urun : urunSonuc, yorum: YorumSonuc, yorumsayi : YorumSayi})
}).catch((err)=>{
res.redirect("/")
})
} else {
res.render(__dirname+'/src/pages/urun.ejs',{title : urunSonuc.baslik, urun : urunSonuc, yorum: YorumSonuc,yorumsayi : YorumSayi});  
}
})
})
})
...
Siparis.find({marketId : id}).count().then((SiparisSayi)=>{
...
res.render(__dirname+'/src/market/dashboard.ejs',{uyarisayi: UyariSayi,uyarilar : UyariSonuc,siparissayi : SiparisSayi,kullanici : kullaniciSonuc, urun : urunlerSonuc, market : marketSonuc, siparisler : SiparisSonuc, title : marketSonuc.isim+" Dashboard"});
} else {
res.redirect('/')
}
})
...
Siparis.find().sort().then((SiparisSonuc)=>{
Siparis.find().count().then((SiparisCountSonuc)=>{
Sikayet.find().sort().then((SikayetSonuc)=>{
res.render(__dirname+'/src/admin/dashboard.ejs',{title : "Admin Dashboard", kullanici : KullaniciSonuc, urun : UrunlerSonuc, market : MarketSonuc, kullanicilar : KullanicilarSonuc, marketSahip : MarketSahipleri, onlineKullanici : onlineUsers, siparisler : SiparisSonuc, siparissayi : SiparisCountSonuc, sikayet : SikayetSonuc})  
...
res.render(__dirname+'/src/signed/settings.ejs',{title : "Kullanıcı Ayarları", kullanici : kullaniciSonuc})
} else {
...
Kullanicilar.findOne({ kullanici_adi: req.body.kullanici_adi }, (err, kullanici) => {
if(kullanici){
res.send(`Bu kullunıcı adı kullanılmakta <a href="/kullanici/ayarlar/${req.cookies.id}">Geri Dön</a>`);
} else {
var id = req.params.id;
Kullanicilar.findByIdAndUpdate(id,{
kullanici_adi : req.body.kullanici_adi,
...
var id = req.params.id;
Kullanicilar.findById(req.cookies.id).then((kullaniciSonuc)=>{
Sepet.find({urunId : id}).count().then((SepetSil)=>{
Siparis.find({urunId : id}).count().then((SiparisSil)=>{
Urunler.findByIdAndDelete(id).then((urunSonuc)=>{
Yorumlar.find({urunId : id}).count().then((YorumSil)=>{
for(var i = 0;i < SepetSil;i++){
Sepet.findOneAndDelete({urunId : id}).then((SepetSilMesaj)=>{
console.log(`${i}.Sepete Eklenen Silindi`)
})
}
for(var i = 0;i < SiparisSil;i++){
Siparis.findOneAndDelete({urunId : id}).then((SiparisSilMesaj)=>{
console.log(`${i}.Ürün Sipariş Silindi`)
})
}
for(var i = 0;i < YorumSil;i++){
Yorumlar.findOneAndDelete({urunId : urunSonuc._id}).then((Mesaj)=>{
...
Sepet.find({kullaniciId : Sonuc._id}).count().then((Sil)=>{
Siparis.find({kullanici : Sonuc.kullanici_adi}).count().then((SilSiparis)=>{
for(var i = 0;i < SilSiparis;i++){
Siparis.findOneAndDelete({kullanici : Sonuc.kullanici_adi}).then((MesajSiparis)=>{
console.log(`${i}.Sipariş Silindi`)
})
}
for(var i = 0;i < Sil;i++){
Sepet.findOneAndDelete({kullaniciId : Sonuc._id}).then((Mesaj)=>{
...
sikayet.save().then((Sonuc)=>{
console.log(`[!] ${KullanicilarSonuc.kullanici_adi} Adlı Kullanıcı Şikayet Edildi`)
console.log(`Kategori : ${req.body.kategori}`)
console.log(`Şikayet : ${req.body.sikayet}`)
res.redirect('/')
})
...
Urunler.find({marketId : id}).count().then((UrunSonuc)=>{
...
Uyari.find({marketId : id}).count().then((UyariSayiSonuc)=>{
